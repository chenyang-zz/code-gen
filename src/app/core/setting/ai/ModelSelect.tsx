...
const ModelSelect = () => {
	...
	const modelExists = (value: string) => {
		return list.some(
			(item) => item.id.toLowerCase() === value.trim().toLowerCase()
		);
	};
	...
	const syncModelList = async (value: string) => {
		setModel(value);
		const store = await Store.load("store.json");
		...
		aiModelList[aiModelList.findIndex((item) => item.key === aiType)] =
			modelConfig;
			...
			{list.length ? (
				<Popover open={open} onOpenChange={setOpen}>
					<PopoverTrigger asChild>
						...
						>
							{model
								? list.find((item) => item.id === model)?.id ??
								  model
								: "Select model"}
