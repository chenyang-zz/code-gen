...
export async function initTagsDb() {
  ...
  `);
	const hasDefaultTag =
		(await db.select<Tag[]>(`select * from tags`)).length === 0;
	if (hasDefaultTag) {
      ...
      true
		...
		const tag = (
			await db.select<Tag[]>(`select * from tags where name = 'Idea'`)
		)[0];
...
export async function getTags() {
	...
	for (const tag of tags) {
		...
		const res = await db.select<{ total: number }[]>(
			`select count(*) as total from marks where tagId = ${tag.id} and deleted = 0`
		);
