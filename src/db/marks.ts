...
export async function initMarksDb() {
	const isExist = await exists("screenshot", {
...
export async function getMarks(id: number) {
	...
	return await db.select<Mark[]>(
		`select * from marks where tagId = ${id} order by createdAt desc`
	);
}
...
export async function getAllMarks() {
	const db = await getDb();
	return await db.select<Mark[]>(
		`select * from marks order by createdAt desc`
	);
}
...
export async function restoreMark(id: number) {
	const db = await getDb();
	return await db.execute(
		`update marks set deleted = 0, createdAt = ${Date.now()} where id = ${id}`
	);
}
...
export async function delMark(id: number) {
	...
	if (res[0].deleted === undefined) {
		await db.execute(
			`alter table marks add column deleted integer default 0`
		);
	}
	return await db.execute(
		`update marks set deleted = 1, createdAt = ${Date.now()} where id = ${id}`
	);
}
