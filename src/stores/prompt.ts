...
interface PromptState {
	...
	createPrompt: (prompt: Omit<Prompt, "id">) => Promise<void>;
...
const usePromptStore = create<PromptState>((set, get) => ({
	promptList: [
		{
			...
			title: "写作助手",
			content:
				"请你扮演一个笔记软件的智能助手，可以参考记录内容，使用 markdown 语法，回答用户的问题。",
			isDefault: true,
		},
	],
	currentPrompt: null,
	...
	initPromptData: async () => {
		...
		if (currentPromptId) {
			const prompt = get().promptList.find(
	...
	savePrompt: async (updatedPrompt) => {
	...
	deletePrompt: async (id) => {
		// 不允许删除默认prompt
		const promptToDelete = get().promptList.find(
			(prompt) => prompt.id === id
		);
		if (promptToDelete?.isDefault) return;

		const promptList = get().promptList.filter(
			(prompt) => prompt.id !== id
		);
		await get().setPromptList(promptList);
		...
		if (currentPrompt && currentPrompt.id === id) {
			const defaultPrompt = get().promptList.find(
