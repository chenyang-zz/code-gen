...
import {
	decodeBase64ToString as giteeDecodeBase64ToString,
	getFiles as getGiteeFiles,
} from "@/lib/gitee";
import type { GiteeFile } from "@/lib/gitee.types";
...
const useArticleStore = create<NoteState>((set, get) => ({
	...
	loadFileTree: async () => {
		...
		if (workspace.isCustom) {
			...
			// 默认工作区
			const isArticleDir = await exists("article", {
			...
			for (const entry of entries) {
				if (entry.isDirectory) {
					...
					if (workspace.isCustom) {
						...
						const pathOptions = await getFilePathOptions(
							dirRelative
						);
						children = (
		...
		const store = await Store.load("store.json");
		const primaryBackupMethod =
			(await store.get<string>("primaryBackupMethod")) || "github";
		...
		if (primaryBackupMethod === "github") {
			...
			const giteeAccessToken = await store.get<string>(
				"giteeAccessToken"
			);
			if (!giteeAccessToken) {
				set({ fileTreeLoading: false });
	...
	newFolderInFolder: async (path: string) => {
		...
		const hasEmptyFolder = currentFolder?.children?.find(
			(item) => item.name === "" && item.isDirectory
		);
		if (hasEmptyFolder) {
			return;
	...
	initCollapsibleList: async () => {
		...
		set({
			collapsibleList: res
				? uniq(res.filter((item) => !item.includes(".md")))
				: [],
		});
	},
	...
	setCollapsibleList: async (path: string, value: boolean) => {
		...
		set({
			collapsibleList: uniq(collapsibleList).filter(
				(item) => !item.includes(".md")
			),
		});
	},
		...
		if (isLocale) {
			try {
				...
				try {
					...
					const store = await Store.load("store.json");
					const primaryBackupMethod =
						(await store.get<string>("primaryBackupMethod")) ||
						"github";

					let content = "";
					if (primaryBackupMethod === "github") {
						content = decodeBase64ToString(
							await getGithubFiles({ path, repo: RepoNames.sync })
						);
					} else {
						content = giteeDecodeBase64ToString(
							await getGiteeFiles({ path, repo: RepoNames.sync })
						);
					}
					set({ currentArticle: content });
					// eslint-disable-next-line @typescript-eslint/no-unused-vars
		...
		const readDirRecursively = async (
			...
			for (const file of files) {
				...
				if (isCustomWorkspace) {
					...
					article = await readTextFile(`${dirPath}/${file.name}`, {
						baseDir: BaseDirectory.AppData,
					});
				}
			...
			for (const dir of directories) {
				...
				const subDirArticles = await readDirRecursively(
					newDirPath,
					newBasePath,
					isCustomWorkspace
				);
