...
const useVectorStore = create<VectorState>((set, get) => ({
	...
	initVectorDb: async () => {
		try {
			...
			const isVectorDbEnabled =
				(await store.get<boolean>("isVectorDbEnabled")) || false;
			const isRagEnabled =
				(await store.get<boolean>("isRagEnabled")) || false;
			const lastProcessTime =
				(await store.get<number>("lastVectorProcessTime")) || null;
